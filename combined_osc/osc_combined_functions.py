...
def reaper_play():
    send_message("/reaper/action/1007", msg)

def reaper_pause():
    send_message("/reaper/action/1008", msg)

def reaper_play_pause():
    send_message("/reaper/action/40073", msg)

def reaper_stop():
    send_message("/reaper/action/1016", msg)

def reaper_play_stop():
    send_message("/reaper/action/40044", msg)

def reaper_marker(marker_no: int):
    #addr = "/action/40161" # Jump to Marker One
    if marker_no <= 10:
        send_message(f"/reaper/action/4016{str(marker_no)}", msg)
    elif marker_no > 10:
...
def lisa_distance(percent): 
    ...
    addr = "/lisa/ext/src/1/d"
    send_message(addr, distance)

def lisa_fx(fx):
    addr = "/lisa/ext/src/1/send/1"
