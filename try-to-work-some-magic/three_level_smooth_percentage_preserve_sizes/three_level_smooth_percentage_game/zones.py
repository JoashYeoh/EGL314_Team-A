...
def configure_zones_for_level(level_number):
    ...
    for zone in ZONES:
        ...
        # Remember whether this zone was already active in the previous level.
        was_active = zone.get("active", False)
        should_be_active = zone["label"] in required_labels

        # Existing zones keep their current radius and percentage OSC state.
        if should_be_active and was_active:
            zone["active"] = True
            continue

        # Newly introduced zones begin at 100%.
        if should_be_active and not was_active:
            ...
            print(
                ...
                f"{zone['label']} introduced at 100%"
            )
            continue

        # Zones not required in the current level stay hidden.
        zone["active"] = False
