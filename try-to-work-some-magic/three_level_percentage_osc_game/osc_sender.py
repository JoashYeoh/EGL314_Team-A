...
def send_zone_percentage(zone_index, percentage_step):
    ...
    osc_tx_gma3.send_message(
        "/gma3/cmd",
        f"Goto Sequence {sequence} Cue {cue}",
    )

    print(
        f"[OSC] {zone_name} {percentage_step}% "
        f"-> Sequence {sequence} Cue {cue}"
    )
